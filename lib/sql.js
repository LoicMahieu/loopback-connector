...
function(columnName, operator, columnValue, propertyValue) {
  ...
  switch (operator) {
    ...
    case 'nlike':
      ...
      break;
    case 'ilike':
      return new ParameterizedSQL('lower(' + sqlExp  + ') LIKE lower(?)', [columnValue]);
    case 'nilike':
      return new ParameterizedSQL('lower(' + sqlExp  + ') NOT LIKE lower(?)', [columnValue]);
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (var i = 0, n = clauses.length; i < n; i++) {
          ...
          var stmtForClause = self._buildWhere(model, clauses[i], asName);
    ...
    if (Array.isArray(where)) {
      ...
      for (var key in where) {
        if (key === 'and' || key === 'or') {
          ...
          dest[key] = dest[key] || {};
          if (where[key]._joinOptions) {
            dest[key]._joinOptions = where[key]._joinOptions
          }
...
SQLConnector.prototype.buildJoins = function(model, where, prevRelName) {
  ...
  var buildOneToMany = function buildOneToMany(relName, modelFrom, keyFrom, modelTo, keyTo, joinWhere,
    ...
    if (relations[relName] && relations[relName].polymorphic) {
      joinWhere[relations[relName].polymorphic.discriminator] = modelDef.model.definition.name;
    }

    if (joinWhere) {
      joinWhere = Object.assign({}, joinWhere)

      const _joinOptions = joinWhere._joinOptions || {}
      delete joinWhere._joinOptions

      if (!_joinOptions.isOptional) {
  ...
  for (var key in where) {
    ...
    if (!rel.modelThrough) {
      ...
      var modelToKey = rel.modelTo.definition.ids()[0].name;
